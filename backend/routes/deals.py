...
def list_deals():
    ...
    supabase = get_supabase_client()
    response = supabase.table("deals").select("id,company_name,currency,description,created_at").execute()
    if getattr(response, "error", None):
        raise HTTPException(status_code=500, detail=str(response.error))
...
def create_deal(payload: DealCreateRequest):
